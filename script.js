...
(function () {
  ...
  function initHero() {
    ...
    // 프레임 이미지 안에 이름/날짜/장소가 디자인으로 박혀있어서,
    // 스크린리더·검색엔진이 읽을 수 있도록 동일한 정보를 숨김 텍스트로 남겨둡니다.
    const srText = $('#heroSrText');
    if (srText) {
      const groomEn = CONFIG.groom.nameEn || CONFIG.groom.name;
      const brideEn = CONFIG.bride.nameEn || CONFIG.bride.name;
      srText.textContent = `Wedding Day. ${groomEn} & ${brideEn} — ${formatDate(CONFIG.wedding.date, CONFIG.wedding.time)}, ${CONFIG.wedding.venue}`;
    }
  }
